...
> **저장소 현황 안내**
> 문서화 작업 시작 시점에 GitHub 저장소에는 실제 소스 코드·데이터·결과 영상이 커밋되어 있지 않았습니다(README 2줄짜리 초기 커밋만 존재). 아래 내용은 1차로 팀이 1학기 동안 Notion에 기록한 회의록(20건)과 기술 문서(13건), 요구사항/발표 자료를 근거로 정리했고, 이후 팀이 자세 피드백 코드 6종(벤치프레스/데드리프트/통합 + 데드리프트 의존 모듈 2개 + phase 라벨링 도구)을 순차적으로 제공해 `src/`에 반영했습니다. **`deadlift_feedback_v2_CLEAN.py`는 의존 모듈을 모두 갖췄지만 전문가 JSON 스키마 불일치(`total_frames` 필드)로 여전히 실행되지 않고, squat 전용 스크립트와 ST-GCN 등 학습 코드(다른 팀원 담당)는 아직 없습니다** — 자세한 현황은 [`docs/system_pipeline.md`](docs/system_pipeline.md) 참고. 2학기 최우선 과제는 이 JSON 스키마 통일과 나머지 코드 이관입니다 — [`docs/second_semester_plan.md`](docs/second_semester_plan.md) 참고.
...
| 데드리프트 자세 피드백 | 부분 구현 (`deadlift_feedback_v2_CLEAN.py` + 의존 모듈 2개 모두 반영됨. 전문가 JSON 스키마 불일치로 실행은 아직 안 됨) | `src/posture_feedback/deadlift/`, `docs/system_pipeline.md` 2절 |
...
- **바벨 검출(실험)**: YOLO-World
- **실제 코드 import로 확인됨** (`src/`): `mediapipe`(`mediapipe.tasks.python.vision`), `opencv-python`(`cv2`), `numpy`(`<2` 제약), `Pillow`(`PIL`), `tkinter`(`phase_labeling.py`의 라벨링 GUI)
- **실제 코드에 연동 흔적 있음(선택적, 키 미설정 시 미사용)**: `anthropic`(Claude API) — `realtime_compare_side.py`가 자세 교정 문구를 LLM으로 생성하는 기능을 갖고 있으나, 저장소 기본값은 `ANTHROPIC_API_KEY = None`이라 규칙 기반 fallback으로 동작
- PyTorch는 ST-GCN/TCN/CTR-GCN 학습에 사실상 필요할 것으로 추정되나, 해당 학습 코드가 아직 저장소에 없어 import로 확인하지는 못했습니다.
- **LLM API 비교 검토(도입 여부 미정, 별도)**: OpenAI, Google Gemini, Anthropic Claude, xAI Grok, Cohere, DeepSeek — Notion에서 비용 비교만 확인됨
...
├─ src/
│  ├─ preprocessing/
│  │  └─ phase_labeling.py  # Start/Bottom/Finish 라벨링 GUI → phase_labels.csv
│  └─ posture_feedback/
│     ├─ benchpress/        # benchpress_feedback_v4.py + expert_benchpress.json 샘플
│     ├─ deadlift/          # deadlift_feedback_v2_CLEAN.py + realtime_compare_side.py + feedback_overlay.py
│     │                       (⚠ JSON 스키마 불일치로 실행은 아직 안 됨, docs/system_pipeline.md 2절)
│     └─ unified/           # unified_feedback_v4.py (3종목 통합 CLI)
│     (action_recognition/ rep_counting/ squat/ utils/ — 아직 코드 없음)
...
의존 모듈(`realtime_compare_side.py`, `feedback_overlay.py`)은 모두 반영되어 import는 성공하지만, `realtime_compare_side.py`의 `load_expert()`가 전문가 JSON에 `total_frames` 필드를 요구하는 반면 `unified_feedback_v4.py`가 생성하는 JSON에는 이 필드가 없어 **`expert_deadlift.json`을 그대로 재사용하면 실행 중 오류가 발생합니다.** JSON에 `total_frames`를 직접 추가하거나 `load_expert()`를 수정해야 합니다 — 자세한 내용은 [`docs/system_pipeline.md`](docs/system_pipeline.md) 2절 참고. 그 전까지 데드리프트는 **`unified_feedback_v4.py --exercise deadlift`로 실행할 것을 권장**합니다.

### 라벨링 도구 (`phase_labeling.py`)

```bash
cd src/preprocessing
python phase_labeling.py
```

`측면_수정본/{benchpress,squat,deadlift}/` 폴더 구조의 영상을 불러와 Start/Bottom/Finish 프레임을 GUI로 라벨링하고 `phase_labels.csv`로 저장합니다(Tkinter GUI, 로컬 데스크톱 전용).
...
- 코드 완성도 문제 (스쿼트 전용 스크립트·ST-GCN 학습 코드 미제공, 전문가 JSON 스키마가 파일마다 달라 `deadlift_feedback_v2_CLEAN.py`가 아직 실행되지 않음, 종목명 표기가 파일마다 다름 — `benchpress` vs `bench`)
