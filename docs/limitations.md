...
- **최초 문서화 시점(2026-07-12 이전)에는 GitHub 저장소(`Capstone-Design---`)에 실제 소스 코드, 데이터, 결과 영상이 전혀 커밋되어 있지 않았습니다.** 이후 팀이 벤치프레스/데드리프트/통합 코드 3종을 제공해 `src/posture_feedback/`에 반영했습니다.
- **여전히 저장소에 없는 것**: 스쿼트 전용 스크립트, `deadlift_feedback_v2_CLEAN.py`가 의존하는 `realtime_compare_side.py`/`feedback_overlay.py`(이게 없으면 데드리프트 스크립트는 import 단계에서 실패), ST-GCN/TCN/CTR-GCN 학습 코드, phase 라벨링 도구(`phase_labeling.py`), `pose_landmarker_lite.task` 모델 파일, 원본 영상/데이터셋. 자세한 목록은 [`system_pipeline.md`](./system_pipeline.md) 4절 참고.
- 반영된 3개 파일도 팀이 "완전한 코드가 아니다"라고 명시했고, 모델 파일·원본 영상이 없어 **실제 실행 검증은 하지 못했습니다**(구문 오류 여부만 `ast.parse`로 확인).
- 세 파일 간 threshold·계산식 불일치([`thresholds.md`](./thresholds.md), [`exercise_metrics.md`](./exercise_metrics.md) 5절)를 하나로 통일하는 것이 2학기 P0 과제입니다 — [`second_semester_plan.md`](./second_semester_plan.md) 참고.
